    ...
    try {
        ...
        if (alreadyLiked) {
            ...
            if (post.author._id != req.userId) {
                ...
                const populatedNotification = await Notification.findById(notification._id).populate("sender","name userName profileImage").populate("receiver","name userName profileImage").populate("post")
...
export const saved = async (req, res) => {
    try {
        ...
        await user.save()
        const populated = await user.populate("saved")
        const userObj = populated.toObject()
        delete userObj.parentEmail
        delete userObj.parentName
