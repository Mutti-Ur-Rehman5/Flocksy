...
export const getCurrentUser=async (req,res)=>{
    try {
        ...
        const user=await User.findById(userId).populate("posts loops posts.author posts.comments story following").select("-parentEmail")
        if(!user){
...
export const editProfile=async (req,res)=>{
    try {
       ...
       const user=await User.findById(req.userId).select("-password -parentEmail")
       if(!user){
        return res.status(400).json({message:"user not found"})
       }
...
export const getProfile = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ userName }).select("-password -parentEmail").populate("posts loops followers following")
    if (!user) {
      return res.status(400).json({ message: "user not found" });
    }
 ...
 targetUser.followers.push(currentUserId)
   if (currentUser._id !=  targetUser._id) {
                             const notification = await Notification.create({
                              ...
                              const populatedNotification = await Notification.findById(notification._id).populate("sender","name userName profileImage").populate("receiver","name userName profileImage")
...
export const getAllNotifications=async (req,res)=>{
  try {
    const notifications=await Notification.find({
      receiver:req.userId
    }).populate("sender","name userName profileImage").populate("receiver","name userName profileImage").populate("post loop").sort({createdAt:-1})
